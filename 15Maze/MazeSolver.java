...
public class MazeSolver{
	...
	public void shp(){
		while(!(front.isEmpty())){
			...
			int c = nexts.getCol();
			board.set(r, c, '.');
			Location x = new Location(r + 1, c, nexts, distance(r + 1, c, board.getStart(), distance(r + 1, c, board.getEnd()), false);
			if (isValid(x)){
				poss.add(x);
			}
			x = new Location(r, c + 1, nexts, distance(r, c + 1, board.getStart(), distance(r + 1, c, board.getEnd()), false);
			if (isValid(x)){
				poss.add(x);
			}
